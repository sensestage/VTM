+ Dictionary {
	...
	changeScalarValuesToDataTypes {
		...
		parseYAMLValue= {arg str;
			...
			{"^<float64>[0-9a-fA-F]{16}$".matchRegexp(str)}
			{
				result = str.drop(9).clump(8).collect({arg it; "0x%".format(it).interpret});
				result = Float.from64Bits(*result);
			}
			{"^<float32>[0-9a-fA-F]{8}$".matchRegexp(str)}
			{
				result = "0x%".format(str.drop(9)).interpret;
